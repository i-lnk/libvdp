...
int             JsonBufferSize
    ...
    for(i=0;i<(int)hRQ->num;i++){
        lens += sprintf(JsonBuffer + lens,"{\"%s\":\"%d\",\"%s\":\"%d\",\"%s\":\"%d\",\"%s\":\"%s\",\"%s\":\"%d\"},",
                        "id"  ,hRQ->dev[i].id,
                        "code",hRQ->dev[i].code,
                        "type",hRQ->dev[i].type,
                        "name",hRQ->dev[i].name,
...
int             JsonBufferSize
	...
	for(int i = 0;i < count;i++){
	    len += sprintf(lps + len,"{\"%s\":\"%04d-%02d-%02d %02d:%02d:%02d\",\"%s\":\"%04d-%02d-%02d %02d:%02d:%02d\",\"%s\":\"%d\",\"%s\":\"%d\",\"%s\":\"%d\"},",
	          	"start",
